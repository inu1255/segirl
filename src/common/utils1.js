 ...
 * 读取文件
 * @param {File} file
 * @param {"ArrayBuffer"|"BinaryString"|"DataURL"|"base64"|"utf8"} [encoding]
 */
...
async function getSeccode() {
	...
	if (m) self.seccode = m[1];
}

self.seccode = "109984457";
...
export async function sougoTranslate(text, isRetry) {
	...
	const from = "auto";
	const to = "zh-CHS";
	const textAfterEscape = text;
	const s = md5("" + from + to + textAfterEscape + self.seccode);
	const payload = {
		from,
		to,
		client: "pc",
	...
	if (!res || res.translate.errorCode === "10") {
		const lastSecode = self.seccode;
		await getSeccode();
		if (self.seccode === lastSecode) return;
