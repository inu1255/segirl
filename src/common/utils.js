			...
			render(h) {
				...
				if (options.showClose != false)
					children.push(h('span', { attrs: { slot: 'footer' }, slot: 'footer' }, [
						h('el-button', { props: { size: 'mini' }, on: { click() { cb = reject, vue.show = false; } } }, options.cancelText || '取消'),
						h('el-button', { props: { size: 'mini', type: 'primary' }, on: { click() { cb = resolve, vue.show = false; } } }, options.submitText || '确定'),
					]));
				return h('el-dialog', {
...
export class DataSource {
	...
	async search(page) {
		...
		if (typeof total === "number")
			this.hasMore = (page + 1) * this.pageSize < this.total;
		else
...
sougo.use(function(config, next) {
	config.url = 'https://fanyi.sogou.com' + config.url;
...
async function getSeccode() {
	const { data } = await sougo.get('/logtrace')
	eval(data)
}
	...
	if (res.dictionary) {
		...
		if (content) {
			if (res.detect.detect == 'en' && content.content) {
				data.dict = content.content.filter(x => x.item).map(x => {
					let ss = []
